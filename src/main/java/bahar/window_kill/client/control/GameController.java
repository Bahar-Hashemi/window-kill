...
public class GameController {
    public void newGame() {
        ...
        User.newInstance(new Epsilon(GameUtil.generateID(), settings.getSpeed()), new MainBoard(GameUtil.generateID()));
        Game.newInstance();
        ...
        User.getInstance().mainBoard.add(User.getInstance().epsilon.getView());
        User.getInstance().epsilon.setLocation(Constants.SCREEN_WIDTH / 2, Constants.SCREEN_HEIGHT / 2);
        MainStage.newScene(); MainStage.add(User.getInstance().mainBoard.getView());
        //
