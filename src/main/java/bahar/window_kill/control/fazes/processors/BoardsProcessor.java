...
public class BoardsProcessor extends GameProcessor {
    ...
    private static void shrinkMainBoard() {
        double minusWidth = Math.min((mainBoard.getWidth() - Constants.MINIMUM_WIDTH) / 2, 0.2);
        double minusHeight = Math.min((mainBoard.getHeight() - Constants.MINIMUM_HEIGHT) / 2, 0.2);
