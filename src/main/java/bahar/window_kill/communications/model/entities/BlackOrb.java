...
public class BlackOrb extends Entity implements BoardOwner, LootDropper{
    GameBoard gameBoard;
    public BlackOrb(boolean isViewable, String id) {
        super(isViewable, id, makeView(isViewable), new Bounds(-30, -30, 30, 30), BlackOrb.class.getName(),
                30, true, null);
        byBoard();
    }
    private static Node makeView(boolean isViewable) {
    ...
    public Entity makeLoot() {
        return new Collectable(isViewable, GameUtil.generateID(), 30, Color.DARKBLUE);
    }
}
